...
// wmgui.clean = function (string, decoder) {
//     // var html = $(str.bold());
//     // html.find('script').remove();
//     // return html.html();
//     // decoder = document.createElement('div');
//     // decoder.innerHTML = string;
//     // console.info(decoder.textContent, decoder.innerHTML);
...
wmgui.show_preloader = function () { preloader.style.display = 'block'; };

wmgui.hide_preloader = function () { preloader.style.display = 'none'; };
...
wmgui.get_interpretation = function (search, facet_names, num_database) {
    ...
    Object.entries(search).forEach(function ([k, val]) {
    ...
    if (!search.formulae && Object.keys(search).length) {

        var add_arity_helper = true,
            arity_helper_html = '';

        wmutils.arity_keys.slice(1).forEach(function (value, num) {
