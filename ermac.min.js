...
wmgui.s_examples=[251737,261485,301194,458778,525194,533193,1005414,1030546,1122968,1215422,1232477,1321212,1406036,1613664,1638591,1640622,1707997,1711681,1722027,1819191,1928624,1933647,1940797];wmgui.mockyear=(new Date).getFullYear();
wmgui.tooltips={advsearch:{el:"advsearch_init_trigger",oleft:-90,otop:60,view_mode:1,text:'Use the &#9776; button for the detailed search by 15+ categories.<br /><span rel="hierarchy">Next</span>'},hierarchy:{el:"hierarchy_trigger",oleft:-90,otop:60,view_mode:1,text:'Use the <i>&mu;</i> button to select physical properties from the curated hierarchy.<br /><span rel="userbox">Next</span>'},userbox:{el:"userbox",oleft:-100,otop:65,view_mode:1,text:'Access your account here and enjoy the full data access.<br /><span rel="close_tooltip">OK</span>'},
interpretation:{el:"right_col",oleft:-70,otop:99,view_mode:2,text:'The entries are grouped into the phases.<br /><span rel="databrowser">Next</span>'},databrowser:{el:"databrowser",oleft:0,otop:200,view_mode:2,text:'Click the particular entry to get more info. Opened lock means open access.<br /><span rel="plots">Next</span>'},plots:{el:"databrowser",oleft:0,otop:500,view_mode:2,text:'Use the graph chart buttons in the footer (at the very bottom of the page).<br /><span rel="close_tooltip">OK</span>'},
ss_axes:{el:"ctxpanel_plots",oleft:60,otop:114,view_mode:2,text:'Click gear icon to sort axes differently.<br /><span rel="visavis" class="forced">Next</span>'},visavis:{el:"right_col",oleft:-150,otop:500,view_mode:2,text:'Fine-tune visualizations with these commands.<br /><span rel="close_tooltip">OK</span>'}};wmgui.tooltip_status=0;wmgui.tooltip_landing="advsearch";
...
$("#usr_api_key").html("Successfully revoked");$("#create_usr_api_key_holder").show()})).fail((function(a,b,c){"abort"!=b&&wmgui.notify('Login error: please, <span class="href relogin">re-login</span>')}))}));$("#login_trigger").click((function(){if(!$(this).data("busy")){$(this).data("busy",!0);$(this).text("In process...");try{wmgui.active_ajax.abort()}catch(a){}wmgui.active_ajax=$.ajax({type:"POST",url:wmgui.login_endpoint,data:{login:$("#login_email").val().trim(),pass:$("#login_password").val()},
beforeSend:wmgui.show_preloader}).always((function(){$("#login_trigger").data("busy",!1);$("#login_trigger").text("Login");wmgui.hide_preloader()})).done((function(a){if(a.error)return wmgui.notify(a.error);if(!a.sid||!a.name||!a.acclogin)return wmgui.notify("Connection to server is lost, please try to <a href=javascript:location.reload()>reload</a>");user_login(a.sid,a.name,a.acclogin,a.admin);$("#userbox").trigger("click")})).fail((function(a,b,c){wmgui.notify("Login unsuccessful")}))}}));$("#restore_trigger").click((function(){if(!$(this).data("busy")){$(this).data("busy",
...
b,c){"abort"!=b&&wmgui.notify("A network error occured. Please, try again")}))}}));$("#account_holder_accpass > span").click((function(){$("#account_pass_change").toggle()}));$("#password_trigger").click((function(){var a=$("#new_password_1").val(),b=$("#new_password_2").val();if(!a||!b)return wmgui.notify("Please, type your new password in both fields");if(a!==b)return wmgui.notify("Passwords do not match (typo?)");if(!$(this).data("busy")){$(this).data("busy",!0);$(this).text("Changing...");try{wmgui.active_ajax.abort()}catch(c){}wmgui.active_ajax=
$.ajax({type:"POST",url:wmgui.password_endpoint,data:{new_password:b,sid:wmgui.sid},beforeSend:wmgui.show_preloader}).always((function(){$("#password_trigger").data("busy",!1);$("#password_trigger").text("Change");wmgui.hide_preloader()})).done((function(a){if(a.error)return wmgui.notify(a.error);user_login(a.sid,a.name,a.acclogin,a.admin);window.location.replace("#start");$("#account_pass_change").hide()})).fail((function(a,b,f){"abort"!=b&&wmgui.notify('Login error: please, <span class="href relogin">re-login</span>')}))}}));
$("#logout_trigger").click((function(){$.ajax({type:"POST",url:wmgui.logout_endpoint,data:{sid:wmgui.sid}}).done((function(a){})).fail((function(a,b,c){}));user_logout();$("#userbox").trigger("click")}));$("#notifybox").on("click","span.relogin",force_relogin);$("#search_holder").keydown((function(a){13==(window.event?a.keyCode:a.which)&&$("#search_trigger").trigger("click")}));$("#control_a").click((function(){re_view_request(0)}));$("#control_b").click((function(){re_view_request(1)}));$("#control_f").click((function(){re_view_request(2)}));
...
a)return!0;k=d==e?' class="ss_z"':"";h+='<li rev="ss_z" rel="z_'+d+'"'+k+">"+c+"</li>"}));$("#ss_x > ul").html(g);$("#ss_y > ul").html(m);"cube"==a&&$("#ss_z > ul").html(h);$("li.sops_x").removeClass("sops_x");$("li.sops_y").removeClass("sops_y");$("li.sops_z").removeClass("sops_z");var l=["x","y","z"];$("span.sops").each((function(){var a=$(this),b=a.text(),c=a.attr("rel");$("#sops_"+c+' > ul > li:contains("'+b+'")').addClass("sops_"+c);l=l.filter((function(a){return a!==c}))}));l.forEach((function(a){$("#sops_"+
a+' > ul > li:contains("none")').addClass("sops_"+a)}));$("#ss_custom_box, #overlay").show()}));$("div.ss_col").on("click","li",(function(){var a=$(this),b=a.attr("rev"),c=a.attr("rel").split("_")[0];c="x"==c?"ss_x":"y"==c?"ss_y":"ss_z";$("#"+b+" > ul > li."+c).removeClass(c);a.addClass(c)}));$("div.sops_col").on("click","li",(function(){var a=$(this),b=a.parent().parent().attr("id"),c=b.split("_")[1];c="x"==c?"sops_x":"y"==c?"sops_y":"sops_z";$("#"+b+" > ul > li."+c).removeClass(c);a.addClass(c)}));$("#tips_trigger").click((function(){wmgui.tooltip_status=
...
27==a&&(a=$("div.cross:visible"),a.length&&a.trigger("click"))}));$(window).bind("hashchange",url_redraw_react);$(window).bind("storage",(function(a){if("wm_reload"==a.originalEvent.key&&(a=a.originalEvent.newValue)&&window[a])window[a]()}))};
var wmgui=window.wmgui||{};
...
switch_control_mode(9,11,"a","c"):b.numeric?(switch_control_mode(1,12,"b",!1),destroy_numericbox(),$.each(get_sliders_ranges(b.numeric,wmgui.numerics),(function(b,a){create_floating_slider.apply(this,a)}))):switch_control_mode(1,10,!1,!1),!b.entry||"S"!=b.entry.substr(0,1)&&"C"!=b.entry.substr(0,1)||-1!=b.entry.indexOf("-")||(c=$("#e__"+b.entry),c.length&&0!=c.attr("data-rank")&&(c.removeClass("busy_entry"),open_context(c,!0)))):(wmgui.fuzzyout?switch_control_mode(1,11,"a",!1):switch_control_mode(0,
10,!1,!1),request_refinement(b,c.estimated_count>wmgui.quick_page_size));b.numeric||(destroy_numericbox(),delete wmgui.search.numeric);b.props&&wmgui.numerics[b.props]&&(c=wmgui.numerics[b.props],create_floating_slider(b.props,c[0],c[1],c[2],c[3],c[4]));d||rebuild_history_box(b,a)})).fail((function(b,a,d){"abort"!=a&&wmgui.notify("Connection to server is lost, please try to <a href=javascript:location.reload()>reload</a>")}))}}
...
'">'+a+"</a>":'<a href="#search/'+escape(a)+'">'+a+"</a>",$("#history ul").prepend("<li>"+wmgui.clean(a)+"</li>"),e.unshift(d),window.localStorage.setItem("wm_search_log_v4",JSON.stringify(e)),8<$("#history ul li").length&&$("#history ul li:last").remove());wmgui.tooltip_status++;1==wmgui.tooltip_status&&setTimeout((function(){show_tooltip(wmgui.tooltips.interpretation)}),3E3)}
...
function switch_view_mode(b){wmgui.view_mode=b;$("div.autocomplete-suggestions").hide();$("#results, #header_entries, #header_phases, #header_articles").hide();$("div.context_msg").hide();close_vibox();1==b?(stop_visavis(),document.title="Materials Platform for Data Science",$("body").addClass("noscroll"),$("#search_box").removeClass("fluid").css("background","none").css("width","590px"),$("div.side_cols").hide(),$("#landing_box, #intro_stats, #subnav, #legend").show(),$("#search_holder").removeClass("prolonged").focus(),
...
function update_dc(){var b=[],a={},d;for(d in wmgui.search)-1<["search_type","phid","numeric"].indexOf(d)||(a[d]=wmgui.search[d],b.push(wmgui.search[d]));b=b.join(" ");var c=0,e=JSON.stringify(a),f='<option value="X" selected>Last search...</option>';$.each(JSON.parse(window.localStorage.getItem("wm_search_log_v4")||"[]"),(function(a,b){if(5<c)return!1;a=[];var d=JSON.stringify(b);if(e==d)return!0;for(var g in b)a.push(b[g]);a=a.join(" ");f+='<option value="'+escape(d)+'">'+a+"</option>";c++}));f+=
'<option value="Y">No comparison</option><option value="Z">Multiple comparison</option>';$("#select_cmp_trigger").empty().append(f)}function show_aetmap(b){(b=wmgui.aets[b])?$("#aetmap").css("height",b[1]+"px").css("background","url("+wmgui.aetmap_addr+") 0 -"+b[0]+"px no-repeat").show():$("#aetmap").hide()}
function render_all_polyhedra(){if($("#all_polyhedra_content").is(":empty")){var b="",a=Object.keys(wmgui.aets);a.sort((function(a,b){var c=a.split("-vertex")[0].split(" ");c=parseInt(c[c.length-1]);var d=b.split("-vertex")[0].split(" ");d=parseInt(d[d.length-1]);return c>d?1:c<d?-1:a.length>b.length?1:a.length<b.length?-1:0}));$.each(a,(function(a,c){a=wmgui.aets[c];b+='<div class="gallery_item" rel="'+c+'"><div style="width:125px;height:'+a[1]+"px;margin:10px auto;background:url("+wmgui.aetmap_addr+
") 0 -"+a[0]+'px no-repeat;"></div><span>'+c+"</span></div>"}));$("#all_polyhedra_content").html(b)}}function assign_edition(){function b(a){a=parseInt(a);wmgui.editions[a]||(a=0);wmgui.edition=a;wmgui.editions[a].css&&wmgui.loadCSS(wmgui.editions[a].css);wmgui.editions[a].actions&&wmgui.editions[a].actions()}$.each(wmgui.editions,(function(a,d){if(wmgui.gui_host==d.prod_url||wmgui.gui_host==d.dev_url)return b(a),!1}));wmgui.edition||b(0)};
var wmgui=window.wmgui||{};
function satisfy_requirements(){$("#notifybox, #preloader").hide();local_user_login();var n="";$.each(JSON.parse(window.localStorage.getItem("wm_search_log_v4")||"[]"),(function(a,d){wmgui.tooltip_status++;if(7<a)return!1;a=[];for(var q in d)a.push(d[q]);a=a.join(" ");var f=!1;$.each(wmgui.inquiries,(function(a,m){if(d[m])return f=!0,!1}));n=f?n+('<li><a href="#inquiry/'+$.param(d)+'">'+a+"</a></li>"):n+('<li><a href="#search/'+escape(a)+'">'+a+"</a></li>")}));$("#history ul").append(n);$.each(JSON.parse(window.localStorage.getItem("bid_history")||
