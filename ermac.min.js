...
wmgui.aug_search_cmd=function(a,b){for(var d=[["elements","formulae"],["formulae","elements"],["lattices","sgs"],["sgs","lattices"],["protos","sgs"],["sgs","protos"],["lattices","protos"],["protos","lattices"]],c=0;c<d.length;c++)if(wmgui.search[d[c][0]]&&a==d[c][1]){delete wmgui.search[d[c][0]];break}var e=!1;$.each(wmgui.inquiries,(function(c,g){if(wmgui.search[g]||g==a)return e=!0,!1}));if(e){var f={};$.each(wmgui.facets,(function(g,c){wmgui.search[c]&&c==a?-1!==wmgui.multi_facets.indexOf(c)?f[c]=
wmgui.search[c]+","+b:f[c]=b:wmgui.search[c]?f[c]=wmgui.search[c]:c==a&&(f[c]=b)}));return"#inquiry/"+$.param(f)}var g="";$.each(wmgui.facets,(function(c,d){wmgui.search[d]&&d==a?g="classes"==d?g+(wmgui.search[d]+" "+b+" "):g+(b+" "):wmgui.search[d]?g+=wmgui.search[d]+" ":d==a&&(g+=b+" ")}));return"#search/"+g.substr(0,g.length-1)};
...
function rebuild_history_box(a,b){var d={};wmgui.facets.forEach((function(b){a[b]&&(d[b]=a[b])}));var c=!1;$.each(wmgui.inquiries,(function(a,b){if(d[b])return c=!0,!1}));var e=[];for(f in d)e.push(d[f].toLowerCase());e.sort();var f=e.join(" ");e=JSON.parse(window.localStorage.getItem("wm_search_log_v4")||"[]");var g=[];8<e.length&&(e=e.slice(0,7));$.each(e,(function(a,b){a=[];for(var c in b)a.push(b[c].toLowerCase());a.sort();g.push(a.join(" "))}));-1==g.indexOf(f)&&(b=c?'<a href="#inquiry/'+$.param(d)+
...
b[1]+"</td><td class=cp>"+d+"</td><td class=c2>"+b[2]+"</td>"+g+"</tr>"}))}d&&(e+=d);return e}
...
(g=d.classes.split(",").map((function(a){return a.trim()})),d.classes=g.join(","));var m=!1;$.each(wmgui.inquiries,(function(a,b){if(d[b])return m=!0,!1}));$.each(c.payload,(function(c,h){if(a.formulae&&"elements"==h.facet)return!0;var n="",p=!1,k={};$.each(d,(function(a,b){if(-1<["search_type","phid","numeric"].indexOf(a)||"elements"==h.facet&&("elements"==a||"formulae"==a)||"props"==h.facet&&"props"==a)return!0;n+=b+" ";k[a]=k[a]?k[a]+(", "+b):b}));a.classes&&"classes"==h.facet&&$.each(g,(function(a,b){if(b==
h.value)return p=!0,!1}));if(p)return!0;h.facet!=e&&(e&&b&&(f+='<li class="fct_'+e+' extd_refine"><a class="extd_refine" rel="'+e+'" href="#">Show more&nbsp;</a></li>'),f+='<li class="new_rfn_facet fct_'+h.facet+'">'+wmgui.facet_names[h.facet]+"</li>",e=h.facet);"props"==h.facet&&(h.value=h.value.replace(/\(|\)/g,"").replace(/\/[\w\-]*/g,""));k[h.facet]=k[h.facet]?k[h.facet]+(", "+h.value):h.value;k.elements&&(k.elements=k.elements.replaceAll(", ","-"));f=m?f+('<li class="fct_'+h.facet+'"><a href="#inquiry/'+
$.param(k)+'">'+h.value+"</a></li>"):f+('<li class="fct_'+h.facet+'"><a href="#search/'+n+h.value+'">'+h.value+"</a></li>");l++}));b&&(f+='<li class="fct_'+e+' extd_refine"><a class="extd_refine" rel="'+e+'" href="#">Show more&nbsp;</a></li>');0==l?display_examples("#examples",!0,!0):($("#refine_col > ul").show().append(wmgui.clean(f)),10>=l&&display_examples("#examples",!1,!1))})).fail((function(a,b,d){"abort"!=b&&(429==a.status?wmgui.notify("Request rate limiting policy: please retry in a few seconds"):
wmgui.notify("Connection to server is lost, please try to <a href=javascript:location.reload()>reload</a>"))}))}
function open_context(a,b){close_vibox();if(a.hasClass("busy_entry")){a.removeClass("busy_entry");$("#ctx_col, #sim_col").hide();try{wmgui.quick_ajax.abort()}catch(g){}}else{var d=a.attr("id").substr(3),c=parseInt(a.attr("data-rank")),e=d.substr(0,1),f=d.split("-")[0];$(".busy_entry").removeClass("busy_entry");a.addClass("busy_entry");"Z"!=e&&"B"!=e&&($("#entryno").html('<a href="#entry/'+f+'">'+f+"</a>"),$("#ctx_col").show(),$("#visualize, div.spinoff_pane, li.d_icon").hide(),open_sim_col(d,e,c),
...
function open_sim_col(a,b,d){$("#sim_col > ul, #sim_legend").empty();$(".sim_col_ctx").hide();$("#sim_col").show();var c={q:a},e=-1<[0,1,4,9].indexOf(d);e&&(c.allpurpose=!0);try{wmgui.quick_ajax.abort()}catch(f){}wmgui.quick_ajax=$.ajax({type:"GET",url:wmgui.sim_endpoint,data:c,beforeSend:function(){$("#sim_preloader").show()}}).always((function(){$("#sim_preloader").hide()})).done((function(c){if(c.error)return wmgui.notify(c.error);var g=0,f="";if(e)$.each(c.out.sim,(function(a,b){if(7==g)return!1;
f=b[4]?f+('<li><a href="#inquiry/'+$.param({elements:b[1],years:b[2],codens:b[3],classes:1==b[4]?"":b[4]})+'">'+b[0]+"</a></li>"):f+('<li><a href="#inquiry/'+$.param({formulae:b[1],years:b[2],codens:b[3]})+'">'+b[0]+"</a></li>");g++}));else if("C"==b||"S"==b)3<d&&($.each(c.out.sim,(function(a,b){if(8==g)return!1;f+='<li><a href="#phase_id/'+b[1]+'">'+b[0]+"</a></li>";g++})),"C"==b&&$("#pd_legend").show());else{var m=wmgui.thumbed_display?$("div.busy_entry > div.gallery_img > div").text().split(";")[0]:
$("tr.busy_entry > td.c2").text().split(";")[0];$.each(c.out.sim,(function(a,b){if(8==g)return!1;f+='<li><a href="#search/'+m+" "+b[1]+'">'+b[0]+"</a></li>";g++}))}g?(e?$("#sim_legend").removeClass("apparent").text("Also studied by these authors:"):"C"==b?$("#sim_legend").removeClass("apparent").text("Phases at the diagram:"):"S"==b?$("#sim_legend").removeClass("apparent").text("The same prototype structures:"):$("#sim_legend").removeClass("apparent").text("Similar "+m+":"),7<g&&$("#sim_trigger").attr("href",
"#interlinkage/"+a).show(),$("#sim_col > ul").html(f)):e?$("#sim_legend").addClass("apparent").text("No cross-links found"):"C"==b&&3==d?$("#sim_legend").addClass("apparent").html('<br /><a href="#entry/'+a.split("-")[0]+'"><img alt="C-entry" src="'+wmgui.static_host+"/pd_thumbs/"+a.split("-")[0]+'.png" /><br /><span>Full phase diagram</span></a>'):"C"==b&&5==d?$("#sim_legend").addClass("apparent").text("No links to other phases found"):"S"==b?$("#sim_legend").addClass("apparent").text("No other structures for this prototype found"):
...
a={total_count:1};$.extend(a,wmgui.search);try{document.getElementById("visavis_iframe").contentWindow.location.replace(get_visavis_url(a))}catch(b){console.error("No iframe access")}}
function manage_visavis(a,b,d){if(wmgui.visavis_terminating||!wmgui.visavis_working)return!1;if(wmgui.search.numeric)return wmgui.visavis_terminating=!0,!1;try{wmgui.active_ajax.abort()}catch(e){}try{wmgui.quick_ajax.abort()}catch(e){}rebuild_visavis();var c={total_count:1};$.extend(c,wmgui.search);try{document.getElementById("visavis_iframe").contentWindow.location.replace(get_visavis_url(c))}catch(e){console.error("No iframe access")}a&&a(b,d);return!0}
...
function user_logout(a){if(wmgui.sid){wmgui.sid=null;$("li.admin").hide();$("div.logged_in").hide();$("div.logged_out").show();window.localStorage.removeItem("wm");try{document.getElementById("visavis_iframe").contentWindow.location.reload()}catch(b){console.error("No iframe access")}communicate_windows("user_logout");a||wmgui.notify("You are logged out")}}
...
function update_dc(){var a=[],b={},d;for(d in wmgui.search)-1<["search_type","phid","numeric"].indexOf(d)||(b[d]=wmgui.search[d],a.push(wmgui.search[d]));a=a.join(" ");var c=0,e=JSON.stringify(b),f='<option value="X" selected>Last search...</option>';$.each(JSON.parse(window.localStorage.getItem("wm_search_log_v4")||"[]"),(function(a,b){if(5<c)return!1;a=[];var d=JSON.stringify(b);if(e==d)return!0;for(var g in b)a.push(b[g]);a=a.join(" ");f+='<option value="'+escape(d)+'">'+a+"</option>";c++}));f+=
'<option value="Y">No comparison</option>';$("#select_cmp_trigger").empty().append(f)}function show_aetmap(a){(a=wmgui.aets[a])?$("#aetmap").css("height",a[1]+"px").css("background","url("+wmgui.aetmap_addr+") 0 -"+a[0]+"px no-repeat").show():$("#aetmap").hide()}
